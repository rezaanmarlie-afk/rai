...
def build_bulk_dashboard(project_key: str, jql: str, max_results: int) -> Dict[str, Any]:
    ...
    for issue in body.get("issues", []):
        ...
        assessed_tickets.append({"issueKey": normalized["issue_key"], "normalized_issue": normalized, "assessment": evaluate_ticket(normalized)})
...
def find_story_point_field_candidates() -> Dict[str, Any]:
    url = f"{JIRA_BASE_URL}/rest/api/3/field"
    response = requests.get(url, headers=jira_auth_headers(), timeout=60)
    result = safe_json_response(response)
    if response.status_code != 200:
        return {"ok": False, "status_code": response.status_code, "configuredStoryPointsField": JIRA_STORY_POINTS_FIELD or None, "error": result.get("jira_response"), "candidates": []}
    ...
    for f in fields:
        ...
        searchable = f"{name} {fid} {schema}".lower()
        if any(k in searchable for k in keywords):
            candidates.append({"id": fid, "name": name, "custom": f.get("custom", False), "schema": schema})
    def score(x):
        n = (x.get("name") or "").lower()
        if n == "story points": return 0
        if n == "story point estimate": return 1
        if "story point" in n: return 2
        if "estimate" in n: return 3
        return 9
    candidates = sorted(candidates, key=score)
    return {"ok": True, "configuredStoryPointsField": JIRA_STORY_POINTS_FIELD or None, "message": "Use the id value, e.g. customfield_10016, in .env as JIRA_STORY_POINTS_FIELD. Restart the app after updating .env.", "candidates": candidates}
...
def jira_story_points_fields():
    return find_story_point_field_candidates()
...
def jira_create_batch(data: Dict[str, Any]):
    ...
    for idx, item in enumerate(items):
        ...
        fields = {
            ...
            "priority": {"name": item.get("priority") or "Medium"},
        }
        ...
        if issue_type.lower() == "story" and story_points_field and item.get("storyPoints") is not None:
            fields[story_points_field] = item.get("storyPoints")
        response = requests.post(f"{JIRA_BASE_URL}/rest/api/3/issue", headers=jira_auth_headers(), json={"fields": fields}, timeout=30)
        if response.status_code in (200, 201):
            created.append(safe_json_response(response)["jira_response"])
        else:
            errors.append({"index": idx, "status_code": response.status_code, "jira_response": safe_json_response(response)["jira_response"]})
    return {"projectKey": project_key, "parentIssueKey": parent_issue_key, "storyPointsFieldUsed": story_points_field or None, "created": created, "errors": errors, "created_count": len(created), "error_count": len(errors)}
