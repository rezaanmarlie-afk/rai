...
def build_capacity_dashboard(
    ...
    if jql_override and str(jql_override).strip():
        ...
        jql = f"{project_clause}{assignee_clause} AND statusCategory != Done ORDER BY updated DESC"

    fields = ["summary", "status", "assignee", "issuetype", "priority", "created", "updated", "parent"]
    if sp_field:
    ...
    while True:
        ...
        for issue in issues:
            ...
            tickets.append({
                ...
                "assignee": (assignee_obj.get("displayName") if isinstance(assignee_obj, dict) else "") or assignee,
                "assigneeAccountId": (assignee_obj.get("accountId") if isinstance(assignee_obj, dict) else "") or assignee_account_id,
            })
    ...
    total_points = round(sum(t["storyPoints"] for t in tickets), 1)
    todo_points = round(sum(t["storyPoints"] for t in tickets if str(t["statusCategory"]).lower() == "to do"), 1)
    in_progress_points = round(sum(t["storyPoints"] for t in tickets if str(t["statusCategory"]).lower() == "in progress"), 1)
    unpointed_count = sum(1 for t in tickets if not t["hasStoryPoints"])

    capacity = float(sprint_capacity_points or 0)
    remaining_capacity = round(capacity - total_points, 1) if capacity > 0 else 0
    ...
    if capacity <= 0:
        ...
        capacity_status = "OVER_ALLOCATED"

    by_project: Dict[str, Dict[str, Any]] = {}
    by_status_category: Dict[str, Dict[str, Any]] = {}
    for t in tickets:
        for key, target in [(t["projectKey"] or "UNKNOWN", by_project), (t["statusCategory"] or "Unknown", by_status_category)]:
            if key not in target:
                target[key] = {"name": key, "ticket_count": 0, "story_points": 0.0, "unpointed": 0}
            target[key]["ticket_count"] += 1
            target[key]["story_points"] = round(target[key]["story_points"] + t["storyPoints"], 1)
            target[key]["unpointed"] += 0 if t["hasStoryPoints"] else 1

    tickets_sorted = sorted(tickets, key=lambda x: (-x["storyPoints"], x["projectKey"], x["issueKey"]))
    return {
        ...
        "jira_fetched_count": fetched_count,
        "jira_pages_read": page_count,
    }
...
def build_team_capacity_dashboard(
    ...
    """Build a team-level capacity and ticket-quality dashboard.

    The manager supplies a roster of users they manage. Each member can have a
    Jira accountId and/or displayName, plus an optional capacityPoints value.
    Done/completed work is excluded by statusCategory and known done status names.
    """
    ...
    if jql_override and str(jql_override).strip():
        ...
        for m in clean_members:
            if m.get("accountId"):
                assignee_values.append(quote_jql_value(m["accountId"]))
            elif m.get("displayName"):
                assignee_values.append(quote_jql_value(m["displayName"]))
        assignee_clause = "assignee in (" + ", ".join(assignee_values) + ")"
        member_clause_used = "accountId/displayName roster"
        jql = f"{project_clause}{assignee_clause} AND statusCategory != Done ORDER BY updated DESC"

    fields = ["summary", "description", "status", "assignee", "issuetype", "labels", "priority", "components", "created", "updated", "reporter", "parent"]
    if sp_field:
        fields.append(sp_field)
    ...
    for m in clean_members:
        if m.get("accountId"):
            member_lookup[m["accountId"]] = m
        if m.get("displayName"):
            member_lookup[m["displayName"].lower()] = m
    ...
    while True:
        ...
        for issue in issues:
            ...
            assignee_obj = f.get("assignee") or {}
            assignee_name = (assignee_obj.get("displayName") if isinstance(assignee_obj, dict) else "") or "Unassigned"
            assignee_account_id = (assignee_obj.get("accountId") if isinstance(assignee_obj, dict) else "") or ""
            member = member_lookup.get(assignee_account_id) or member_lookup.get(assignee_name.lower()) or {"displayName": assignee_name, "accountId": assignee_account_id, "capacityPoints": default_capacity_points}
    ...
    def cap_status(util: float, cap: float) -> str:
        if cap <= 0:
            return "UNKNOWN"
        if util <= 80:
            return "AVAILABLE"
    ...
    for m in clean_members:
        key = m.get("accountId") or m.get("displayName", "").lower()
        by_member[key] = {
    ...
    for t in tickets:
        key = t.get("assigneeAccountId") or t.get("assignee", "").lower()
        ...
        if row is None:
            row = by_member.setdefault(key or t.get("assignee"), {
                ...
                "active_ticket_count": 0, "total_story_points": 0.0, "todo_story_points": 0.0, "in_progress_story_points": 0.0,
                "unpointed_ticket_count": 0, "quality_score_total": 0.0, "quality_ticket_count": 0, "average_quality_score": 0,
                "pass_count": 0, "partial_count": 0, "fail_count": 0, "utilization_percent": 0, "remaining_capacity_points": float(default_capacity_points or 0), "capacity_status": "UNKNOWN"
            })
        pts = float(t.get("storyPoints") or 0)
        row["active_ticket_count"] += 1
        row["total_story_points"] = round(row["total_story_points"] + pts, 1)
        ...
        for issue in t.get("qualityIssues") or []:
            by_quality_gap[issue] = by_quality_gap.get(issue, 0) + 1
        pk = t.get("projectKey") or "UNKNOWN"
        if pk not in by_project:
            by_project[pk] = {"name": pk, "ticket_count": 0, "story_points": 0.0, "quality_score_total": 0.0, "quality_ticket_count": 0, "average_quality_score": 0}
    ...
    for row in by_member.values():
        ...
        total = float(row.get("total_story_points") or 0)
        row["utilization_percent"] = round((total / cap) * 100, 1) if cap > 0 else 0
        row["remaining_capacity_points"] = round(cap - total, 1) if cap > 0 else 0
        row["capacity_status"] = cap_status(row["utilization_percent"], cap)
    ...
    total_points = round(sum(float(t.get("storyPoints") or 0) for t in tickets), 1)
    ...
    utilization = round((total_points / total_capacity) * 100, 1) if total_capacity > 0 else 0
    ...
    return {
        ...
        "summary": {
            ...
            "total_story_points": total_points,
            "remaining_capacity_points": round(total_capacity - total_points, 1) if total_capacity > 0 else 0,
            "utilization_percent": utilization,
            "capacity_status": cap_status(utilization, total_capacity),
            "unpointed_ticket_count": sum(1 for t in tickets if not t.get("hasStoryPoints")),
        ...
        "by_member": sorted(by_member.values(), key=lambda x: (-x.get("utilization_percent", 0), x.get("displayName") or "")),
        ...
        "tickets": sorted(tickets, key=lambda x: (-x.get("storyPoints", 0), x.get("assignee") or "", x.get("issueKey") or "")),
        "jira_fetched_count": fetched_count,
        "jira_pages_read": page_count,
    }
...
def assignee_capacity(payload: Dict[str, Any]):
    ...
    sprint_capacity_points = float(payload.get("sprintCapacityPoints") or payload.get("capacityPoints") or 20)
    max_results = int(payload.get("maxResults") or payload.get("max_results") or 0)
    if not assignee and not assignee_account_id:
        return JSONResponse(status_code=400, content={"error": "assignee or assigneeAccountId is required"})
    try:
        return build_capacity_dashboard(assignee, project_keys, story_points_field_id, sprint_capacity_points, jql_override, max_results, assignee_account_id)
...
def team_capacity(payload: Dict[str, Any]):
    ...
    story_points_field_id = (payload.get("storyPointsFieldId") or payload.get("story_points_field_id") or "").strip()
    jql_override = (payload.get("jql") or payload.get("jqlOverride") or "").strip()
    default_capacity_points = float(payload.get("defaultCapacityPoints") or payload.get("capacityPoints") or 20)
    max_results = int(payload.get("maxResults") or payload.get("max_results") or 0)
    try:
        return build_team_capacity_dashboard(members, project_keys, story_points_field_id, default_capacity_points, jql_override, max_results)
    except ValueError as e:
        return JSONResponse(status_code=400, content={"error": str(e)})
    except RuntimeError as e:
        return JSONResponse(status_code=500, content={"error": str(e)})
