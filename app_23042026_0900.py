...
def merge_human_input_into_rewrite(
    summary: str,
    ...
    if systems_involved:
        scope_points = dedupe_keep_order(
            scope_points + [f"Involved system/domain: {x}" for x in systems_involved], 8
        )

    dependency_points = infer_dependency_points(summary, existing_description)
    dependency_points = dedupe_keep_order(dependency_points + dependencies_input + approvals, 8)

    success_points = infer_success_points(summary, existing_description)
    success_points = dedupe_keep_order(success_points + success_input + validation_steps, 8)

    acceptance_points = infer_acceptance_criteria(summary, existing_description)
    if validation_steps:
        acceptance_points = dedupe_keep_order(
            acceptance_points + [f"Given implementation is completed, when validation is performed, then {x}." for x in validation_steps[:2]],
            6
        )
    ...
    risk_points = detect_risk(existing_description)["evidence"]
    risk_points = dedupe_keep_order(risk_points + risks_input, 8)
    if not risk_points:
        risk_points = ["Execution must be controlled safely, with required approvals and validation evidence retained."]
...
""".strip()
    ...
    return {
        ...
        "generated_sections": {
            ...
            "scope": scope_points,
            "dependencies": dependency_points,
            "success_criteria": success_points,
            "acceptance_criteria": acceptance_points,
            "definition_of_ready": dor_points,
            "definition_of_done": dod_points,
            "risks": risk_points,
...
def normalize_section_items(section_name: str, chunks: List[str]) -> List[str]:
    ...
    for chunk in chunks:
        ...
        if section_name in {
            "Scope", "Dependencies", "Success Criteria", "Acceptance Criteria",
            "Definition of Ready", "Definition of Done", "Risks / Controls"
        }:
            extracted = extract_bullets_from_text(chunk, 20)
            if extracted:
                items.extend(extracted)
            else:
                items.extend(split_sentences(chunk))
        else:
...
def auto_clean_and_normalize_ticket(summary: str, description: str, issue_type: str = "Story") -> Dict[str, Any]:
    ...
    if not risks:
        risks = detect_risk(description)["evidence"] or [
...
def build_bulk_dashboard(project_key: str, jql: str, max_results: int) -> Dict[str, Any]:
    fields = [
...
def bulk_results_to_csv_rows(result: Dict[str, Any], weak_only: bool = False) -> List[List[str]]:
    ...
    for t in result.get("tickets", []):
        ...
        has_acceptance = (
            assessment["evidence"]["acceptance"]["found"]
            or assessment["evidence"]["dor_dod"]["dor_found"]
            or assessment["evidence"]["dor_dod"]["dod_found"]
            or assessment["evidence"]["success_criteria"]["found"]
        )
        ...
        rows.append([
            ...
            ni["priority"],
            "Yes" if (ni["description"] or "").strip() else "No",
            str(len(ni["labels"] or [])),
            str(len(ni["components"] or [])),
            "Yes" if has_acceptance else "No",
            " | ".join(assessment["issues"][:3]),
            " | ".join(assessment["strengths"][:3]),
        ])
...
def auto_clean_ticket(payload: Dict[str, Any]):
    ...
    return auto_clean_and_normalize_ticket(summary, description, issue_type)
...
def auto_clean_live_ticket(payload: Dict[str, Any]):
    ...
    cleaned = auto_clean_and_normalize_ticket(
        normalized["summary"],
        normalized["description"],
        normalized["issue_type"] or "Story"
    )
...
def enhance_with_human_input(payload: Dict[str, Any]):
    ...
    enhanced = merge_human_input_into_rewrite(
        normalized["summary"],
        normalized["description"],
        normalized["issue_type"] or "Story",
        human_input
...
def bulk_dashboard_csv(
    projectKey: str = Query(default=DEFAULT_PROJECT),
    jql: Optional[str] = Query(default=None),
    maxResults: int = Query(default=25),
):
    ...
    rows = bulk_results_to_csv_rows(result, weak_only=False)
    output = csv_stream(rows)
...
def bulk_dashboard_weak_csv(
    ...
    resolved_jql = jql or f"project = {project_key} ORDER BY created DESC"
    result = build_bulk_dashboard(project_key, resolved_jql, maxResults)
    rows = bulk_results_to_csv_rows(result, weak_only=True)
    output = csv_stream(rows)
