...
def find_section_snippet(text: str, headings: List[str], window: int = 1200) -> str:
    ...
    for heading in headings:
        ...
        if idx != -1:
            snippet = text[idx: idx + window].strip()
            return snippet
    return ""
...
def detect_acceptance_evidence(description: str, explicit_ac: Any = None) -> Dict[str, Any]:
    ...
    if all(token in lower for token in ["given", "when", "then"]):
        gwt_hits = find_sentences_with_keywords(description, ["given", "when", "then"], max_hits=6)
        evidence.extend(gwt_hits)
    ...
    unique_evidence = list(dict.fromkeys([e.strip() for e in evidence if e.strip()]))

    return {
        "found": bool(unique_methods),
...
def detect_dependencies_evidence(description: str, explicit_dependencies: Any = None) -> Dict[str, Any]:
    ...
    dependency_keywords = [
        ...
        "access and credentials", "3rd-party", "vendor dependencies",
        "points of contact", "upstream", "downstream", "approval", "approvals",
        "security", "architecture sign-off", "firewall", "access"
    ]
...
def detect_business_context(description: str) -> Dict[str, Any]:
    keywords = [
        ...
        "operational impact", "business need", "current", "results in", "outcome"
    ]
...
def detect_scope(description: str) -> Dict[str, Any]:
    keywords = [
        ...
        "capabilities", "poc scope", "feature", "implements", "demonstrates"
    ]
    hits = find_sentences_with_keywords(description, keywords, max_hits=10)
...
def detect_risk(description: str) -> Dict[str, Any]:
    keywords = [
        ...
        "operational risk", "security", "safe", "guardrails", "negative-path"
    ]
    hits = find_sentences_with_keywords(description, keywords, max_hits=10)
...
def detect_success_criteria(description: str) -> Dict[str, Any]:
    keywords = [
        ...
        "reduction", "mttr", "effort", "evidence-based", "go/no-go", "thresholds"
    ]
    hits = find_sentences_with_keywords(description, keywords, max_hits=10)
...
def evaluate_ticket(issue_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Acceptance / DoR / DoD / Success Criteria
    ...
    # Small uplift for strong mature ticket structure
    maturity_bonus = 0
    if dor_found:
        maturity_bonus += 4
    if dod_found:
