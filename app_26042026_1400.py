...
# -----------------------------------------------------------------------------
# Manager capacity / cross-project allocation view
# -----------------------------------------------------------------------------
...
def get_likely_story_points_field_id(preferred_field: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        keywords = ["story points", "story point estimate", "story point", "estimate"]
        candidates = []
        for f in fields:
            name = str(f.get("name", ""))
            fid = str(f.get("id", ""))
    ...
    return {"field_id": "", "source": "not_found", "warning": "No Story Points / estimate custom field was discovered. Configure JIRA_STORY_POINTS_FIELD or enter the field id manually."}
...
def build_capacity_dashboard(assignee: str, project_keys: List[str], story_points_field_id: str = "", sprint_capacity_points: float = 20, jql_override: str = "", max_results: int = 0) -> Dict[str, Any]:
    ...
    if jql_override and str(jql_override).strip():
        ...
        safe_assignee = quote_jql_value(assignee)
        jql = f"{project_clause}assignee = {safe_assignee} AND statusCategory != Done ORDER BY updated DESC"
    ...
    while True:
        ...
        for issue in issues:
            ...
            category = status_category_name(issue)
    ...
    for t in tickets:
        for container, key in [(by_project, t["projectKey"] or "UNKNOWN"), (by_status, t["statusCategory"] or "UNKNOWN")]:
            if key not in container:
                container[key] = {"name": key, "ticket_count": 0, "story_points": 0.0}
            container[key]["ticket_count"] += 1
            container[key]["story_points"] += t["storyPoints"]
    for bucket in list(by_project.values()) + list(by_status.values()):
        bucket["story_points"] = round(bucket["story_points"], 1)
    return {
        "assignee": assignee,
        ...
        "by_status_category": sorted(by_status.values(), key=lambda x: x["name"]),
        "tickets": sorted(tickets, key=lambda x: (-x["storyPoints"], x["projectKey"], x["issueKey"])),
        "jira_fetched_count": fetched_count,
        "jira_pages_read": page_count,
        "allMatchingTicketsIncluded": (not max_results or int(max_results) <= 0),
    }
...
def assignee_capacity(payload: Dict[str, Any]):
    ...
    max_results = int(payload.get("maxResults") or payload.get("max_results") or 0)
    if not assignee:
        return JSONResponse(status_code=400, content={"error": "assignee is required"})
    try:
        return build_capacity_dashboard(assignee, project_keys, story_points_field_id, sprint_capacity_points, jql_override, max_results)
